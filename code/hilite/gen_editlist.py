...
def load_model():
    ...
    return {'gra': model, 'pca': pca_model}
...
def main():
    # sample usage:
    #   highlight.py --max_time 180 --save_edits --save_clips --max_clips 5  -t fade video1.mp4 video2.mp4 video3.mp4
    ...
    parser.add_argument('-d', '--dir_video', metavar='{dir}', type=str, default='.',
            help='output directory for final highlight video')

    parser.add_argument('--dir_edits', metavar='{dir}', type=str,  default=None,
            help='output directory for edit lists (default: {dir_video})')
    ...
    parser.add_argument('--save_edits', action='store_true',
            help='save the generated edit list as CSV file')

    parser.add_argument('--save_clips', action='store_true',
            help='save the generated clips used to make final video')

    parser.add_argument('--max_time', metavar='{seconds}', type=int, nargs=1, default=180,
            help='max total time of final highlight video (in seconds)')

    parser.add_argument('--num_clips', metavar='N', type=int, nargs=1,
            help='exact number of highlights to include in edit list)')

    parser.add_argument('--max_clips', metavar='N', type=int, nargs=1, default=10,
            help='maximum number of clips to include in edit list)')

    parser.add_argument('--min_clips', metavar='N', type=int, nargs=1, default=3,
            help='minimum number of clips to include in edit list)')

    parser.add_argument('-t', '--transition', metavar='{effect}', choices=['fade', 'none'], default='fade',
            help='enable scene transitions between clips (only "fade" supported)')

    parser.add_argument('files', metavar='{files}', nargs='+',
            help='input filenames of full-length video games to process')

    args = parser.parse_args()

    # for most args, just use args.{option}
    dir_video = args.dir_video
    dir_edits = args.dir_edits if(args.dir_edits) else args.dir_video
    dir_temp = args.dir_temp  if(args.dir_temp) else args.dir_edits

    print("pwd: {}".format(os.getcwd()))
    print("args:", args)
    print("  args.dir_video  : %s " % args.dir_video)
    ...
    print("  args.transition : %s " % args.transition)

    print("using:")
    print("  dir_video  : %s " % dir_video)
    print("  dir_edits  : %s " % dir_edits)
    print("  dir_temp   : %s " % dir_temp)

    for d in [dir_edits, dir_video, dir_temp]:
    ...
    for arg in args.files:
        ...
        for filename in sorted(set(list(glob.glob(arg)) + [arg])):
            print("=== file: ", filename )
            video_path = join(dir_video, filename)
