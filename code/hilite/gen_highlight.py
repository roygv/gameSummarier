...
def concat_clips(event_data, transitions=None, filenames=None, dir_out=".", dir_input=None):
    ...
    for index, row in event_data.iterrows():
        ...
        if isfile(video_path):
            videoClip = VideoFileClip(video_path).subclip(row['secondOffset'],row['secondOffset']+row['timeToCapture'])
    ...
    if video_path:
        ...
        print("** Error: no clips found to create highlight matching patterns: %s", ['*' if not filenames else filenames])
...
def main():
    ...
    parser.add_argument('-i', '--dir_input', metavar='{dir}', type=str, default=None,
            help='input directory of full game videos (default: use path from edit list)')

    parser.add_argument('-d', '--dir_out', metavar='{dir}', type=str, default='.',
            help='output directory for final highlight video (default: "."')
    ...
    parser.add_argument('-s', '--save_edits', action='store_true',
            help='save the edit list (default: delete after processed) [not implemented]')
